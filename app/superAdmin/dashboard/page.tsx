...
// SuperAdmin Dashboard - app/page-path: /superAdmin/dashboard
...
export default function AdminDashboard() {
	...
	const handleUpdateStatus = async () => {
		...
		const receiverName = editReceiverName.trim();
		const weight = editWeightKg.trim();
		if (!receiverName) {
	...
	return (
		<div className="space-y-4">
			...
			<Card id="shipments">
				<div className="flex items-center justify-between mb-4">
					...
					<div className="flex items-center gap-3">
						<div className="relative">
							<button className="flex items-center gap-2 px-4 py-2 rounded-md border border-[#E6EEF5] bg-white text-sm shadow-sm"></button>
						</div>
					</div>
				</div>
				...
				<div className="hidden sm:block overflow-hidden rounded-md border border-[#EEF2F6] bg-white">
					<div className="overflow-x-auto">
						<table className="w-full min-w-[980px] text-xs">
							<thead>
								<tr className="bg-[#EFEFEF] text-gray-950">
									<th className="px-6 py-4 text-left font-medium">Tracking ID</th>
									{isSuperAdminArea ? <th className="px-6 py-4 text-left font-medium">Origin</th> : null}
							...
							<tbody className="divide-y divide-gray-100">
								{shipmentsLoading ? (
									<tr>
										<td colSpan={shipmentTableColumns} className="px-6 py-8 text-center text-[#94A3B8]">
											Loading shipments...
										</td>
									</tr>
								) : shipments.length > 0 ? (
									shipments.map((shipment) => (
										<tr key={shipment.id} className="align-middle">
											<td className="px-6 py-6 text-xs font-semibold text-[#0F2940]">
												{shipment.trackingNumber || '—'}
											</td>
											{isSuperAdminArea ? (
												<td className="px-6 py-6 text-xs text-[#475569]">{shipment.origin || '—'}</td>
											) : null}
											<td className="px-6 py-6 text-xs text-[#475569]">{shipment.destination || '—'}</td>
											{isSuperAdminArea ? (
												<td className="px-6 py-6 text-xs text-[#475569] whitespace-nowrap">
											...
											<td className="px-6 py-6 text-right text-xs text-[#94A3B8] whitespace-nowrap">
											...
											<td className="px-6 py-6 text-right">
												<Button
													...
													className="px-3 py-2 bg-black text-white hover:bg-[#29292a]"
												>
													Update
												</Button>
											</td>
								...
								<div className="mt-4 space-y-3">
									{isSuperAdminArea ? (
										<div>
											...
											<p className="text-sm text-[#475569] wrap-break-word">{shipment.origin || '—'}</p>
										</div>
									) : null}
									<div>
									...
									{isSuperAdminArea ? (
										<div>
											<p className="text-xs text-[#94A3B8]">Receiver Phone</p>
											<p className="text-sm text-[#475569] wrap-break-word">{shipment.receiverPhone || '—'}</p>
										</div>
									) : null}
									<div>
							...
							{isModalLoading ? (
								...
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
									...
									<div className="border border-[#E5E7EB] rounded-xl p-6">
										<h3 className="text-lg font-semibold text-[#1F2937] mb-4">Shipment Details</h3>
										...
										<div className="grid grid-cols-2 gap-y-6 text-sm">
											...
											<div>
												...
												<p className="font-medium">
													{shipmentDetails?.receiver_phone || shipmentDetails?.receiver_contact?.phone || '—'}
												</p>
											</div>
											...
											<div>
												...
												<p className="font-medium">{shipmentDetails?.destination || selectedShipment.destination || '—'}</p>
											</div>
											...
											<div className="col-span-2">
												...
												<p className="font-medium">{shipmentDetails?.items_description || selectedShipment.description || '—'}</p>
											</div>
												...
												<p className="font-medium">
													{formatDateOnly(
														shipmentDetails?.shipment_date ||
															selectedShipment.shipmentDate ||
															shipmentDetails?.created_at ||
											...
											<div>
												...
												<p className="font-medium">{shipmentDetails?.updated_at ? formatDate(shipmentDetails.updated_at) : '—'}</p>
											</div>
										</div>
									</div>
